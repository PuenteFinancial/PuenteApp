...
| 409 | `hold_cannot_clear` | Ops hold-release refused (2026-09-14): the hold is operator-releasable and the row has not moved, but releasing it cannot clear it — the submit job re-measures the same condition on the next sweep. Today: an `fx_drift` hold whose quote is past `FX_MAX_QUOTE_AGE_MINUTES` (quote age only grows). NOT `conflict` — refreshing never helps; raise the bound or cancel + refund |
...
// GET /v1/ops/transfers/:id → 200 (abridged)
{
  ...
  "holdRelease": { "blocker": "stale_quote", "quoteAgeMinutes": 6900, "maxQuoteAgeMinutes": 240 },  // whether Release hold could clear THIS row; null unless the hold is fx_drift (the one reason with two arms)
...
| 1 interlock | Bridge unreachable (transport error, timeout, Bridge 5xx) | 502 `provider_unavailable` | The check did not run; nothing was written. Retry in a minute — the one refusal whose instruction is "retry". |
| 1 interlock | Bridge 4xx (does not recognize our ref) | 409 `principal_not_returned` | Not transient. `details[0].issue` = `bridge_lookup_failed; bridge=http_<status>`. Read the Bridge dashboard. |
