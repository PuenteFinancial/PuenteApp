...
Driving a transfer under the manual processor, end to end (the full run, buttons and
break-glass CLI both: `manual-funding-run.md`):
...
2. Deposit coordinates are **automatic at confirm** (funding-ops slice 3): the worker's
   `funding.onramp_prepare` job creates the Bridge onramp for the exact total (`on_behalf_of`
   the sender's Bridge customer, destination = treasury wallet, `client_reference_id` = the
   transfer id) and attaches its coordinates — the pay step renders them, reference code
   included, seconds after confirm. Verify on the ops board; the row's Attach button is the
   break-glass when the job dead-ends (Sentry `onramp-prepare-*`).
3. The sender pays at their bank. **Release policy (Joshua, 2026-08-18): the payout may be
   ...
   The ops board's **Release payout** button (`kind: funded`) is that assertion (see below).
   **Deposit landed** (`cleared` + float top-up, same onramp ref) still waits for the money to
   actually LAND in the wallet — the books record arrival, not intent, and the pair is what
   keeps the daily wallet-float reconciliation clean.
