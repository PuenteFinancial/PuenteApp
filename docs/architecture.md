...
| Cron | Railway | Scheduled in the worker process: `payout.sweep` (1-min, re-enqueues lost/stale submits), `payout.poll` (N-min Bridge reconciliation backstop), stale-`PENDING_PAYMENT` reconcile (5-min; 30-min no-webhook → `PAYMENT_FAILED`), idempotency-key purge (daily 04:00), `ledger.correction-watch` (hourly; rolling Reg E correction-loss sum → `loss-correction-threshold` page). |
