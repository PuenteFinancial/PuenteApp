...
- [~] **KYC:** ~~Bridge requires **us** to perform KYC and hand verified identity to them. Provider: **Sumsub**.~~ **SUPERSEDED TWICE.** Sumsub was never integrated: 2026-07-13 chose Bridge-hosted KYC (Persona) for the MVP, and the K lane (2026-08-27 → 2026-09-03) moved verification to **Stripe embedded components at first send**, with the sender's DOB and tax ID relayed once to Bridge and never stored. The original instinct was right about one thing — Bridge does require us in the middle — which is exactly what the K6 relay is. See `docs/plans/kyc-at-first-send.md` and the 2026-09-03 entries in `decisions.md`. `IdentityVerifier` survives only as the swap seam.
...
- [~] **Confirm OFAC / KYC division with Bridge** — who screens, what data we pass, where the handoff sits. **Mostly answered by the K6 probe (2026-09-02):** we pass name, DOB, address, and tax ID to Bridge's Customers API and **Bridge runs its own sanctions, PEP, blocklist, and database checks** from it; the tax ID is mandatory (`missing.all_of`), the document is optional (`any_of` with `database_lookup`). Still open with Bridge: the `rejection_reasons` vocabulary, the duplicate-identity shape, whether a failed create burns the signed agreement, and whether customer-level `approved` implies the `spei` endorsement (#192).
