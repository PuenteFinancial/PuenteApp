...
/**
 * The phone `registerLinkUser` gets: E.164. The API now serves the login phone
 * as E.164 (K7a — see toE164Nanp in packages/shared for the history: GoTrue
 * stores it bare and Stripe's consumer sign-up 400s on that shape), so this is
 * belt-and-braces over the same helper. NANP only (the sender is US); null
 * means "don't call the SDK with it".
 */
export function linkPhoneFor(raw: string): string | null {
  return toE164Nanp(raw)
}
