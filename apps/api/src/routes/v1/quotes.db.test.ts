...
describe.skipIf(!runDb)('quotes (integration, local Supabase)', () => {
  ...
  beforeEach(async () => {
    ...
    await db.query(
      `truncate table public.ledger_entries, public.ledger_transactions,
       ...
       public.cancellation_requests, public.deposit_instructions, public.sender_notices, public.ops_actions, public.transfers,
  ...
  describe('domain constraints', () => {
    it.each([
      ...
      ['negative margin', { margin_minor: '-1' }],
      ['margin not below send (#193: transfer_payable must stay positive)',
        { margin_minor: '19801' }],
