...
// The dispute interlock moved to dispute-interlock.ts when the refund tail
// needed it too (refunds.ts is upstream of this module, so it could not import
// from here). Imported for use below AND re-exported, so existing importers and
// tests are unaffected by the move.
export { type DisputeVerdict } from './dispute-interlock.js'
export { verifyFundingNotDisputed }
